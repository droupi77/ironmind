...
const CAT_FLAT = Object.keys(CATALOGUE).flatMap((m,mi) => CATALOGUE[m].map((e,i) => Object.assign({},e,{muscle:m,id:mi*100+i})));
...
const FAIS_BY_M = {
  pecs: ["Sterno-costal - Milieu","Claviculaire - Haut","Costal - Bas"],
  dos: ["Grand Dorsal - Largeur","Haut du Dos - Épaisseur","Érecteurs - Bas du dos"],
  epaules: ["Deltoïde Antérieur","Deltoïde Latéral","Deltoïde Postérieur"],
  biceps: ["Chef Long - Peak","Chef Court - Épaisseur","Brachial - Avant-bras"],
  triceps: ["Chef Long - OVERHEAD","Chef Latéral et Médial","Composé - Masse globale"],
  quadri: ["Vastes - Composé lourd","Vastes - Secondaire","Rectus Femoris - ISO","Unilatéral - Amplitude"],
  ischio: ["Hip Hinge - Étiré","Leg Curl - Priorité","Leg Curl - Complémentaire"],
  fessiers: ["Grand Fessier - Force","Grand Fessier - Amplitude","Moyen Fessier - Stabilité","Grand Fessier - Isolation"],
  mollets: ["Gastrocnémien - Genou tendu","Soléaire - Genou fléchi"],
  abdos: ["Grand Droit - Anti-ext","Grand Droit - Flexion","Grand Droit - Force","Obliques - Rotation"],
};

// ─── BUILDER (séries directes plafonnées, science 2024 + mode avancé faisceau) ──
// Cibles de volume en séries directes/semaine
const VOL = { low: 6, mid: 11, high: 17 };       // volume par MUSCLE (mode simple)
const VOL_F = { low: 3, mid: 6, high: 9 };       // volume par FAISCEAU (mode avancé)
const VOL_FLOOR = 4;    // plancher : tout muscle actif ≥ 4 séries
const VOL_CAP   = 20;   // plafond dur : jamais de junk volume au-delà

function buildProgram(opts) {
  // opts: { prio, faisceauPrio, advanced, type, sets, days, split, gender, lieu }
  const { type, sets, days, split, gender, lieu } = opts;
  ...
  const tM   = { force:.85, hypertrophie:1, endurance:1.10 }[type];
  const dM   = { 2:.60, 3:.72, 4:.86, 5:1 }[days] || 1;
  const MIN_EX = 4, MAX_EX = 6;
  const ok = e => home ? e.loc !== "gym" : e.loc !== "home";

  // Exercices obligatoires (santé + couverture faisceau clé)
  const MAND = {
    gym: {
  ...
  // Clamp volume entre plancher et plafond
  const clampVol = v => Math.max(VOL_FLOOR, Math.min(VOL_CAP, Math.round(v * tM * dM)));

  // ── Calcul du volume cible PAR FAISCEAU ──
  // En mode simple : on répartit le volume du muscle sur ses faisceaux
  // En mode avancé : volume fixé directement par faisceau
  function faisceauTargets(mid) {
    const cat = (CATALOGUE[MK[mid]] || []).filter(ok);
    if (!cat.length) return {};
    const faisList = [...new Set(cat.map(e => e.f))];
    const targets = {};

    if (advanced && opts.faisceauPrio) {
      // Mode avancé : priorité par faisceau (échelle réduite, faisceau < muscle)
      for (const f of faisList) {
        const p = opts.faisceauPrio[f];
        if (p) targets[f] = Math.max(2, Math.min(12, Math.round(VOL_F[p] * tM * dM)));
      }
    } else {
      // Mode simple : volume muscle réparti équitablement sur les faisceaux actifs
      const p = opts.prio[mid];
      if (!p) return {};
      const muscleVol = clampVol(VOL[p]);
      // répartition équitable, le reste va aux premiers faisceaux
      const base = Math.floor(muscleVol / faisList.length);
      let rem = muscleVol - base * faisList.length;
      for (const f of faisList) {
        targets[f] = base + (rem-- > 0 ? 1 : 0);
      }
    }
    return targets;
  }

  // ── Sélection des exercices pour un muscle, selon volume par faisceau ──
  function pool(mid) {
    const cat = (CATALOGUE[MK[mid]] || []).filter(ok);
    if (!cat.length) return [];
    const targets = faisceauTargets(mid);
    const mandNames = MAND[home ? "home" : "gym"][mid] || [];
    const selected = [];

    // Pour chaque faisceau ciblé, choisir assez d'exos pour couvrir le volume
    for (const [f, volTarget] of Object.entries(targets)) {
      if (volTarget < 1) continue;
      const faisExos = cat.filter(e => e.f === f);
      if (!faisExos.length) continue;
      const nbExos = Math.max(1, Math.min(Math.ceil(volTarget / sets), faisExos.length));
      // prioriser les mandatory
      const ordered = [
        ...faisExos.filter(e => mandNames.includes(e.name)),
        ...faisExos.filter(e => !mandNames.includes(e.name)),
      ];
      // séries par exo = volume du faisceau réparti, borné entre 2 et sets+1
      const perEx = Math.max(2, Math.min(sets + 1, Math.round(volTarget / nbExos)));
      for (let i = 0; i < nbExos; i++) {
        const e = ordered[i];
        if (e && !selected.find(s => s.name === e.name)) {
          const exSets = perEx;
          selected.push({
            name:e.name, muscle:MK[mid], faisceau:e.f, exType:e.et,
            note:e.note, alts:e.alts, gif:e.gif, sets:exSets,
            reps:(RT2[e.et] && RT2[e.et][type]) || "8-12",
            rir: e.et.indexOf("compound")===0 ? "2-3" : "0-1",
          });
        }
      }
    }
  ...
  // ── Calcul du volume effectif réel produit (séries directes par muscle) ──
  function computeVolume() {
    const vol = {};
    for (const m of MC) {
      vol[m.id] = G(m.id).reduce((a,e) => a + (e.sets||3), 0);
    }
    return vol;
  }

  // ── Distribution dans les jours (anti-répétition par faisceau) ──
  // sépare les exos d'un muscle en N "vagues" pour des séances différentes
  function splitWaves(exos, nWaves) {
    const waves = Array.from({length:nWaves}, () => []);
    // trier par faisceau pour distribuer les angles différents sur des jours différents
    const byF = {};
    exos.forEach(e => (byF[e.faisceau] = byF[e.faisceau] || []).push(e));
    let w = 0;
    // distribuer faisceau par faisceau en round-robin
    for (const f of Object.keys(byF)) {
      for (const e of byF[f]) {
        waves[w % nWaves].push(e);
        w++;
      }
    }
    return waves;
  ...
  const SORT = ["Quadriceps","Ischio","Fessiers","Pectoraux","Dos","Épaules","Triceps","Biceps","Mollets","Abdominaux"];
  // Ordre par type d'exercice : composés lourds d'abord (frais), isolation en dernier (fatigue OK)
  const ET_ORDER = {compound_heavy:0, compound_secondary:1, poly_isolation:2, isolation:3};
  const sortEx = arr => [...arr].sort(function(a,b){
    var ta = ET_ORDER[a.exType]!==undefined ? ET_ORDER[a.exType] : 2;
    var tb = ET_ORDER[b.exType]!==undefined ? ET_ORDER[b.exType] : 2;
    if (ta !== tb) return ta - tb;                    // 1. type d'exercice (lourd→isolation)
    return SORT.indexOf(a.muscle) - SORT.indexOf(b.muscle); // 2. groupe musculaire
  });

  // Cap exos par séance + garantir min
  function capDay(exos) {
    return exos.slice(0, MAX_EX);
  }

  // ── Garantir MIN_EX exos par séance (complète avec exos non utilisés du même groupe) ──
  function enforceMin(dayObj, candidateMuscles) {
    if (dayObj.exercises.length >= MIN_EX) return dayObj;
    const usedNames = new Set(dayObj.exercises.map(e=>e.name));
    const usedFais  = new Set(dayObj.exercises.map(e=>e.faisceau));
    // piocher dans tout le catalogue des muscles de cette séance, faisceaux non couverts d'abord
    const pool2 = candidateMuscles.flatMap(mid => {
      const cat = (CATALOGUE[MK[mid]]||[]).filter(ok);
      return cat.map(e=>({
        name:e.name, muscle:MK[mid], faisceau:e.f, exType:e.et,
        note:e.note, alts:e.alts, gif:e.gif, sets:Math.max(2,Math.min(sets,3)),
        reps:(RT2[e.et]&&RT2[e.et][type])||"8-12",
        rir: e.et.indexOf("compound")===0?"2-3":"0-1",
      }));
    });
    // d'abord faisceaux non couverts
    for (const e of pool2) {
      if (dayObj.exercises.length>=MIN_EX) break;
      if (!usedNames.has(e.name) && !usedFais.has(e.faisceau)) {
        dayObj.exercises.push(e); usedNames.add(e.name); usedFais.add(e.faisceau);
      }
    }
    // puis n'importe quel exo non utilisé
    for (const e of pool2) {
      if (dayObj.exercises.length>=MIN_EX) break;
      if (!usedNames.has(e.name)) { dayObj.exercises.push(e); usedNames.add(e.name); }
    }
    dayObj.exercises = sortEx(dayObj.exercises);
    return dayObj;
  }

  // muscles candidats par type de séance (pour enforceMin)
  function dayMuscles(dayName) {
    if (dayName.includes("Push")||dayName.includes("Pectoraux")) return ["pecs","epaules","triceps"];
    if (dayName.includes("Pull")||dayName.includes("Dos")) return ["dos","biceps"];
    if (dayName.includes("Legs")||dayName.includes("Jambes")) return fem?["fessiers","ischio","quadri","mollets"]:["quadri","ischio","fessiers","mollets"];
    if (dayName.includes("Épaules")) return ["epaules","triceps","biceps"];
    if (dayName.includes("Bras")) return ["biceps","triceps"];
    if (dayName.includes("Upper")) return ["pecs","dos","epaules","triceps","biceps"];
    return MC.map(m=>m.id);  // full body
  }
  ...
  if (split === "fullbody" || days <= 2) {
    const order = fem
      ? ["fessiers","ischio","quadri","pecs","dos","epaules","triceps","biceps","mollets","abdos"]
      : ["quadri","ischio","fessiers","pecs","dos","epaules","triceps","biceps","mollets","abdos"];
    const buckets = Array.from({length:days}, () => []);
    // distribuer chaque exo de chaque muscle en round-robin sur les jours
    let idx = 0;
    for (const id of order) {
      for (const e of G(id)) {
        buckets[idx % days].push(e);
        idx++;
      }
    }
    result = {
      title:`${tN} · Full Body · ${days}j/sem`,
      days: buckets.map((ex,i)=>({
        name:`Jour ${i+1} — Full Body`, focus:"Corps entier",
        exercises: capDay(sortEx(ex)),
  ...
  // ── PPL ──
    ...
    const pushM = ["pecs","epaules","triceps"];
    const pullM = ["dos","biceps"];
    const legM  = fem ? ["fessiers","ischio","quadri","mollets"] : ["quadri","ischio","fessiers","mollets"];

    if (days === 3) {
      result = {
        title:`${tN} · PPL · 3j/sem`,
        days:[
          {name:"Jour 1 — Push", focus:"Pectoraux · Épaules · Triceps", exercises:capDay(sortEx(pushM.flatMap(G)))},
          {name:"Jour 2 — Pull", focus:"Dos · Biceps", exercises:capDay(sortEx(pullM.flatMap(G)))},
          {name:"Jour 3 — Legs", focus:"Jambes complètes", exercises:capDay(sortEx(legM.flatMap(G)))},
        ],
      };
    } else if (days === 4) {
      // Push / Pull / Legs / Upper — Upper pioche les exos restants non utilisés
      const pushEx = capDay(sortEx(pushM.flatMap(G)));
      const pullEx = capDay(sortEx(pullM.flatMap(G)));
      const usedNames = new Set([...pushEx,...pullEx].map(e=>e.name));
      const upperPool = [...pushM,...pullM].flatMap(G).filter(e=>!usedNames.has(e.name));
      result = {
        title:`${tN} · PPL · 4j/sem`,
        days:[
          {name:"Jour 1 — Push", focus:"Pectoraux · Épaules · Triceps", exercises:pushEx},
          {name:"Jour 2 — Pull", focus:"Dos · Biceps", exercises:pullEx},
          {name:"Jour 3 — Legs", focus:"Jambes complètes", exercises:capDay(sortEx(legM.flatMap(G)))},
          {name:"Jour 4 — Upper", focus:"Haut du corps (complément)", exercises:capDay(sortEx(upperPool))},
        ],
      };
    } else {
      // 5j : Push A/B, Pull A/B via splitWaves (anti-répétition garantie)
      const pushW = splitWaves(pushM.flatMap(G), 2);
      const pullW = splitWaves(pullM.flatMap(G), 2);
      result = {
        title:`${tN} · PPL · 5j/sem`,
        days:[
          {name:"Jour 1 — Push A", focus:"Pectoraux · Épaules · Triceps", exercises:capDay(sortEx(pushW[0]))},
          {name:"Jour 2 — Pull A", focus:"Dos · Biceps", exercises:capDay(sortEx(pullW[0]))},
          {name:"Jour 3 — Legs", focus:"Jambes complètes", exercises:capDay(sortEx(legM.flatMap(G)))},
          {name:"Jour 4 — Push B", focus:"Angles secondaires haut", exercises:capDay(sortEx(pushW[1]))},
          {name:"Jour 5 — Pull B", focus:"Angles secondaires dos/bras", exercises:capDay(sortEx(pullW[1]))},
        ],
      };
    }
  }
  ...
  // ── BRO SPLIT ──
    ...
    const slots = [
      {l:"Pectoraux", f:"Pectoraux · Triceps", m:["pecs","triceps"]},
      {l:"Dos",       f:"Dos · Biceps",        m:["dos","biceps"]},
      {l:"Épaules",   f:"Épaules",             m:["epaules"]},
      {l:"Jambes",    f:"Jambes complètes",    m:fem?["fessiers","ischio","quadri","mollets"]:["quadri","ischio","fessiers","mollets"]},
      {l:"Bras",      f:"Biceps · Triceps",    m:["biceps","triceps"]},
    ];
    result = {
      title:`${tN} · Bro Split · ${days}j/sem`,
      days: Array.from({length:days},(_,i)=>{
        const s = slots[i % slots.length];
        return {name:`Jour ${i+1} — ${s.l}`, focus:s.f, exercises:capDay(sortEx(s.m.flatMap(G)))};
      }),
    };
  }

  // ── Appliquer enforceMin sur toutes les séances ──
  result.days.forEach(d => enforceMin(d, dayMuscles(d.name)));

  // ── Volume effectif (attaché au résultat pour affichage) ──
  result.volume = computeVolume();
  result.targets = Object.fromEntries(MC.map(m=>{
    const t = faisceauTargets(m.id);
    return [m.id, Object.values(t).reduce((a,b)=>a+b,0)];
  }));
  // Alertes
  result.warnings = [];
  for (const m of MC) {
    const v = result.volume[m.id];
    if (v > 0 && v < VOL_FLOOR) result.warnings.push(`${MK[m.id]}: ${v} séries (< plancher ${VOL_FLOOR})`);
    if (v > VOL_CAP) result.warnings.push(`${MK[m.id]}: ${v} séries (> plafond ${VOL_CAP}, junk volume)`);
  }

  result.days = result.days.filter(d => d.exercises.length);
  return result;
}


// ─── THEME ────────────────────────────────────────────────────────────────────
const C={bg:"#0a0e13",bg2:"#101820",s:"rgba(255,255,255,.04)",s2:"rgba(255,255,255,.06)",bd:"rgba(255,255,255,.09)",ac:"#ff5436",tx:"#e8edf2",mu:"#5a6b7a",lo:"#3b82f6",mi:"#f59e0b",hi:"#ef4444",ok:"#22c55e"};
...
const gif=name=>{const e=CAT_FLAT.find(e=>e.name===name);return e?e.gif:null;};
const alts=name=>{const e=CAT_FLAT.find(x=>x.name===name);return e?e.alts:[];};
      ...
      {!list.length
        ...
        : list.map((n,i)=>{const alt=CAT_FLAT.find(e=>e.name===n); return <div key={i} style={{background:C.s2,border:`1px solid ${C.bd}`,borderRadius:10,marginBottom:10}}>
            <div style={{padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
              <div style={{flex:1}}><div style={{fontWeight:700,fontSize:13,marginBottom:3}}>{n}</div>{(alt&&alt.note)&&<div style={{fontSize:11,color:C.mu,lineHeight:1.5}}>{alt.note}</div>}</div>
...
// ─── SESSION TRACKER ──────────────────────────────────────────────────────────
function SessionTracker({program,history,onSave,onUpdateProg,tSec,tRun,tapTimer,fmt2}){
  ...
  var exos=ov[di]||day.exercises;
  var tot=exos.reduce(function(a,e){return a+(e.sets||3);},0);
  var dn=Object.values(done).filter(Boolean).length;
  var pct=Math.round(dn/tot*100)||0;
  const h2w=n=>(history||[]).filter(s=>s.exercises&&s.exercises.some(x=>x.name===n)).sort((a,b)=>b.ts-a.ts).slice(0,2).map(s=>({ts:s.ts,sets:(s.exercises.find(x=>x.name===n)||{sets:[]}).sets}));
  const swap=(ei,n)=>{const nx=[...exos],d=CAT_FLAT.find(e=>e.name===n);nx[ei]={...nx[ei],name:n,muscle:(d&&d.muscle)||nx[ei].muscle,note:(d&&d.note),alts:(d&&d.alts)||[],faisceau:(d&&d.f)||nx[ei].faisceau,exType:(d&&d.et)||nx[ei].exType};setOv(function(p){var n=Object.assign({},p);n[di]=nx;return n;});onUpdateProg&&onUpdateProg(di,nx);setD(function(d2){var nd=Object.assign({},d2);for(var si2=0;si2<(nx[ei].sets||3);si2++){delete nd[ei+'-'+si2];}return nd;});setL(function(l){var nl=Object.assign({},l);Object.keys(nl).forEach(function(k){if(k.indexOf(ei+'-')===0)delete nl[k];});return nl;});setSw(null);};
  ...
  var swEx=sw?sw.ex:null, swEi=sw?sw.ei:0;
  return <div>
    {sw ? <SwapModal ex={swEx} onSwap={function(n){swap(swEi,n);}} onClose={function(){setSw(null);}}/> : null}
    ...
    {exos.map((ex,ei)=>{
      var s=ex.sets||3;
      var ed=Array.from({length:s},function(_,si){return done[ei+'-'+si];}).filter(Boolean).length;
      var g=ex.gif||gif(ex.name);
      var h=h2w(ex.name);
      return <div key={ei} style={{background:C.s,border:`1px solid ${C.bd}`,borderRadius:10,marginBottom:10,overflow:"hidden"}}>
        ...
        <div style={{padding:"8px 13px"}}>
          ...
          {Array.from({length:s},(_,si)=>{const k=ei+'-'+si,d2=!!done[k];return <div key={si} style={{display:"grid",gridTemplateColumns:"28px 1fr 1fr 34px",gap:6,marginBottom:5,alignItems:"center"}}>
            ...
            <input type="number" placeholder={(ex.reps||'8').split('-')[0]} disabled={d2} value={logs[ei+'-'+si+'-r']||""} onChange={e=>setL(function(l){var k=ei+'-'+si+'-r';var n=Object.assign({},l);n[k]=e.target.value;return n;})} style={inp(d2)}/>
            <input type="number" placeholder="kg" disabled={d2} value={logs[ei+'-'+si+'-k']||""} onChange={e=>setL(function(l){var k=ei+'-'+si+'-k';var n=Object.assign({},l);n[k]=e.target.value;return n;})} style={inp(d2)}/>
            <button onClick={()=>setD(function(d3){var n=Object.assign({},d3);n[k]=!d3[k];return n;})} style={{width:32,height:32,borderRadius:6,border:`2px solid ${d2?C.ok:C.bd}`,background:d2?C.ok:"transparent",color:d2?"#000":C.mu,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>{d2?"✓":"○"}</button>
          </div>;})}
        </div>
      </div>;
    })}
  ...
  {/* Chronomètre — après le bouton pour ne rien bloquer */}
  <div onClick={tapTimer} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:tRun?"rgba(232,255,71,.07)":"rgba(34,34,38,.5)",border:`2px solid ${tRun?C.ac:tSec>0?C.mu:C.bd}`,borderRadius:12,padding:"12px 16px",marginTop:10,cursor:"pointer",userSelect:"none",transition:"all .2s"}}>
    <div style={{fontSize:10,color:tRun?C.ac:C.mu,textTransform:"uppercase",letterSpacing:1.5,fontWeight:700}}>{tRun?"⏱ En cours":"⏱ Chrono pause"}</div>
    <div style={{fontWeight:900,fontSize:28,color:tRun?C.ac:tSec>0?C.mu:C.tx,fontVariantNumeric:"tabular-nums",letterSpacing:2}}>{fmt2(tSec)}</div>
    <div style={{fontSize:9,color:C.mu,textTransform:"uppercase",letterSpacing:1}}>{tRun?"Tap = stop":tSec>0?"Tap = reset":"Tap = start"}</div>
  </div>
  </div>;
}
...
function StatsPage({history}){
  ...
  if(!history||!history.length) return <div style={{textAlign:"center",padding:"60px 20px"}}><div style={{fontSize:44,marginBottom:12}}>📊</div><div style={{fontWeight:700,fontSize:17,marginBottom:6}}>Aucune donnée</div><div style={{color:C.mu,fontSize:13}}>Complète des séances pour voir ta progression</div></div>;
  const map={};
  history.forEach(s=>s.exercises&&s.exercises.forEach(ex=>{if(!map[ex.name])map[ex.name]={name:ex.name,muscle:ex.muscle,pts:[]};const b=(ex.sets||[]).filter(s=>s.kg&&s.done).sort((a,bb)=>parseFloat(bb.kg)-parseFloat(a.kg))[0];if(b)map[ex.name].pts.push({ts:s.ts,v:parseFloat(b.kg),reps:b.reps});}));
...
export default function App(){
  ...
  const [faisceauPrio,setFaisceauPrio]=useState({});
  ...
  const tP=(id,lv)=>setPrio(function(p){var n=Object.assign({},p);n[id]=p[id]===lv?undefined:lv;return n;});
  const tF=(f,lv)=>setFaisceauPrio(function(p){var n=Object.assign({},p);n[f]=p[f]===lv?undefined:lv;return n;});
  const go1=()=>{if(!gender||!lieu)return;setPrio(DPRIO[gender]||{});setPhase(1);};

  const updateProg=(dayIdx,newExos)=>{
    const updated={...activeP,days:activeP.days.map((d,i)=>i===dayIdx?{...d,exercises:newExos}:d)};
    setActiveP(updated);
    setProgs(prev=>prev.map(p=>p.id===activeP.id?updated:p));
  };
  const doGen=()=>{const p=buildProgram({prio,faisceauPrio,advanced,type,sets,days,split,gender,lieu});if(!p.days||!p.days.length){alert("Sélectionne au moins un groupe ou faisceau musculaire.");return;}p.id=Date.now();p.title=pName||p.title;setGen(p);};
  const saveG=()=>{if(!gen)return;setProgs(p=>[...p,gen]);setGen(null);setPName("");setPhase(0);setGender(null);setLieu(null);setAdvanced(false);setFaisceauPrio({});setTab("programs");};
  ...
  const addEx=ex=>{if(cDays[cDay].exercises.find(e=>e.name===ex.name))return;setCDays(d=>d.map((day,i)=>i===cDay?{...day,exercises:[...day.exercises,{name:ex.name,muscle:ex.muscle,faisceau:ex.f,exType:ex.et,note:ex.note,gif:ex.gif,alts:ex.alts,sets,reps:(RT[ex.et]&&RT[ex.et].hypertrophie)||"8-12"}]}:day));};
  ...
  const cEN=(cDays[cDay]&&cDays[cDay].exercises.map(e=>e.name))||[];
  ...
  return <div style={{background:C.bg,minHeight:"100vh",width:"100%",backgroundImage:"linear-gradient(170deg,#101820,#0a0e13 60%)"}}><div style={{color:C.tx,fontFamily:"'Space Grotesk',system-ui,sans-serif",minHeight:"100vh",maxWidth:680,margin:"0 auto",display:"flex",flexDirection:"column",overflowX:"hidden",paddingBottom:"86px"}}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,600;9..144,900&family=Bebas+Neue&family=Space+Grotesk:wght@400;500;600;700&display=swap');*{box-sizing:border-box}html,body{background:#0a0e13;margin:0;padding:0}input{outline:none;font-family:inherit}input:focus{border-color:#ff5436!important}button{font-family:inherit;cursor:pointer}.serif{font-family:'Fraunces',serif}.bebas{font-family:'Bebas Neue',sans-serif}`}</style>

    {/* HEADER */}
    <div style={{padding:"52px 22px 16px",position:"sticky",top:0,zIndex:10,background:"linear-gradient(180deg,#0d141b 70%,transparent)"}}>
      <div className="serif" style={{fontWeight:900,fontSize:30,color:C.tx,letterSpacing:"-.5px",lineHeight:1}}>Iron<span style={{color:C.ac}}>Mind</span></div>
      <div style={{fontSize:10,color:C.mu,letterSpacing:2,marginTop:4,textTransform:"uppercase"}}>{tab==="programs"?"Mes programmes":tab==="generate"?"Générateur":tab==="custom"?"Créer un programme":tab==="session"?"Séance en cours":"Progression"}</div>
    </div>

    <div style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"20px 16px 40px",WebkitOverflowScrolling:"touch"}}>
        ...
        {!gen?<>
          ...
          {phase===1&&<>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div style={{fontSize:10,textTransform:"uppercase",letterSpacing:2,color:C.mu}}>{advanced?"Priorités par faisceau":"Priorités musculaires"}</div>
              <button onClick={()=>setAdvanced(a=>!a)} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${advanced?C.ac:C.bd}`,background:advanced?"rgba(232,255,71,.1)":C.s2,color:advanced?C.ac:C.mu,fontSize:11,fontWeight:700,cursor:"pointer"}}>{advanced?"⚙ Mode avancé":"○ Mode simple"}</button>
            </div>
            {advanced?(
              <div style={{marginBottom:20}}>
                <div style={{fontSize:11,color:C.mu,marginBottom:12,lineHeight:1.5,fontStyle:"italic"}}>Cible chaque faisceau individuellement pour corriger un point faible précis.</div>
                {MC.map(m=>(
                  <div key={m.id} style={{marginBottom:14}}>
                    <div style={{fontWeight:700,fontSize:13,marginBottom:6,color:C.ac}}>{m.e} {m.n}</div>
                    {(FAIS_BY_M[m.id]||[]).map(f=>(
                      <div key={f} style={{display:"flex",alignItems:"center",gap:8,marginBottom:5,background:C.s2,border:`1px solid ${faisceauPrio[f]?(function(){var x=PRIOS.find(p=>p[0]===faisceauPrio[f]);return x?x[2]:C.bd;})():C.bd}`,borderRadius:8,padding:"7px 10px"}}>
                        <div style={{flex:1,fontSize:11}}>{f}</div>
                        <div style={{display:"flex",gap:3}}>{PRIOS.map(([lv,lb])=><button key={lv} style={pill(faisceauPrio[f]===lv,lv)} onClick={()=>tF(f,lv)}>{lb}</button>)}</div>
                      </div>
                    ))}
                  </div>
                ))}
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:20}}>
                {MC.map(m=><div key={m.id} style={{background:C.s2,border:`2px solid ${prio[m.id]?(function(){var f=PRIOS.find(x=>x[0]===prio[m.id]);return f?f[2]:C.bd;})():C.bd}`,borderRadius:10,padding:"10px 12px"}}>
          ...
          {gen.volume&&<div style={{background:C.s,border:`1px solid ${C.bd}`,borderRadius:10,padding:"12px 14px",marginBottom:14}}>
            <div style={{fontSize:10,textTransform:"uppercase",letterSpacing:1.5,color:C.mu,marginBottom:10,fontWeight:700}}>📊 Volume hebdo par muscle</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6}}>
              {MC.filter(m=>gen.volume[m.id]>0).map(m=>{
                var v=gen.volume[m.id];
                var col=v<4?C.hi:v>20?C.mi:v>=10?C.ok:C.ac;
                var lab=v<4?"insuffisant":v>20?"junk volume":v>=10?"optimal":"léger";
                return <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 0"}}>
                  <span style={{fontSize:12}}>{m.e} {m.n}</span>
                  <span style={{display:"flex",alignItems:"center",gap:6}}>
                    <span style={{fontWeight:800,fontSize:14,color:col}}>{v}</span>
                    <span style={{fontSize:9,color:C.mu,minWidth:62,textAlign:"right"}}>{lab}</span>
                  </span>
                </div>;
              })}
            </div>
            {gen.warnings&&gen.warnings.length>0?<div style={{marginTop:10,paddingTop:10,borderTop:`1px solid ${C.bd}`}}>
              {gen.warnings.map((w,i)=><div key={i} style={{fontSize:11,color:C.mi,marginBottom:3}}>⚠ {w}</div>)}
            </div>:<div style={{marginTop:10,paddingTop:10,borderTop:`1px solid ${C.bd}`,fontSize:11,color:C.ok}}>✓ Tous les volumes sont dans la zone optimale</div>}
          </div>}
          {gen.days.map((d,di)=><div key={di} style={{background:C.s,border:`1px solid ${C.bd}`,borderRadius:10,marginBottom:10,overflow:"hidden"}}>
            ...
            <div style={{padding:"8px 14px"}}>{d.exercises.map((ex,ei)=><div key={ei} style={{padding:"8px 0",borderBottom:ei<d.exercises.length-1?`1px solid ${C.bd}`:"none"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <div style={{textAlign:"right",flexShrink:0,marginLeft:10}}><span style={{fontWeight:800,fontSize:15,color:C.ac}}>{ex.sets}×</span><span style={{fontSize:11,color:C.mu,marginLeft:4}}>{ex.reps} reps</span>{ex.rir?<div style={{fontSize:9,color:C.mu,marginTop:1}}>RIR {ex.rir}</div>:null}</div>
              </div>
            </div>)}</div>
          </div>)}
      ...
      {tab==="custom"&&<div>
        ...
        {cDays[cDay]&&cDays[cDay].exercises.length>0&&<div style={{background:C.s,border:`1px solid ${C.bd}`,borderRadius:10,padding:"11px 13px",marginBottom:12}}>
      ...
      {/* SÉANCE */}
      {tab==="session"&&(activeP
        ?<SessionTracker program={activeP} history={hist} onSave={s=>setHist(p=>[...p,s])} onUpdateProg={updateProg} tSec={tSec} tRun={tRun} tapTimer={tapTimer} fmt2={fmt2}/>
    ...
    {/* NAVIGATION BAS */}
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:30,background:"rgba(13,20,27,.92)",backdropFilter:"blur(16px)",borderTop:`1px solid ${C.bd}`,maxWidth:680,margin:"0 auto",display:"flex",justifyContent:"space-around",padding:"10px 0 26px"}}>
      {NAV.map(([id,label,icon])=><button key={id} onClick={()=>setTab(id)} style={{border:"none",background:"transparent",display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"4px 14px",color:tab===id?C.ac:C.mu}}>
